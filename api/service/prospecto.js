...
Prospecto.methods(['get', 'post', 'put', 'delete'])

//sempre que dar update retornará o objeto novo e não o objeto antigo / e fazer validações também ao fazer update
Prospecto.updateOptions( { new: true, runValidators: true } )
Prospecto.after('post', sendErrorsOrNext).after('put', sendErrorsOrNext)
...
Prospecto.route('count', function(req, res, next) {
  ...
  Prospecto.count(function(error, value) {
    if(error) {
